...
export function AccountDashboard({
  ...
  return (
    <div className="account-sections">
      ...
      <section className="account-card" aria-labelledby="perfil">
        <h2 id="perfil">Tus datos</h2>
        <form className="lead-form" onSubmit={saveProfile}>
          <label>
            Nombre y apellido
            <input name="name" defaultValue={account.name} required minLength={2} />
          </label>
          <label>
            Teléfono / WhatsApp
            <input name="phone" defaultValue={account.phone ?? ""} inputMode="tel" />
          </label>
      ...
      <section className="account-card" aria-labelledby="preferencias">
        <h2 id="preferencias">Qué estás buscando</h2>
        <p className="detail-meta">
          Sirve para que el vendedor te proponga unidades acordes. No calcula una
          cuota ni reserva nada.
        </p>
        <form className="lead-form" onSubmit={savePreferences}>
          <label>
            ...
            <input
              name="budget"
              inputMode="numeric"
              defaultValue={preferences.budgetCents ? String(preferences.budgetCents / 100) : ""}
              placeholder="8.000.000"
            />
          </label>
          <label>
            Cuota máxima que podés pagar
            <input
              name="monthly"
              inputMode="numeric"
              defaultValue={
                preferences.maxMonthlyPaymentCents
                  ? String(preferences.maxMonthlyPaymentCents / 100)
                  : ""
              }
              placeholder="600.000"
            />
          ...
          <label>
            Marcas que te interesan
            <input
              name="makes"
              defaultValue={preferences.preferredMakes.join(", ")}
              placeholder="Toyota, Volkswagen"
            />
          </label>
          <fieldset className="account-choices">
            <legend>Tipo de vehículo</legend>
            {BODY_TYPES.map((type) => (
              <label key={type} className="consent-check">
                <input
                  type="checkbox"
                  name={`body-${type}`}
                  defaultChecked={preferences.preferredBodyTypes.includes(type)}
                />{" "}
                {type === "auto" ? "Auto" : type === "suv" ? "SUV" : "Pick-up"}
              </label>
            ))}
          </fieldset>
          ...
          <label>
            Marca
            <input name="currentMake" defaultValue={String(currentVehicle.make ?? "")} />
          </label>
          <label>
            Modelo
            <input name="currentModel" defaultValue={String(currentVehicle.model ?? "")} />
          </label>
          <label>
            Año
            <input
              name="currentYear"
              inputMode="numeric"
              defaultValue={currentVehicle.year ? String(currentVehicle.year) : ""}
            />
          </label>
          <label>
            Kilómetros
            <input
              name="currentKm"
              inputMode="numeric"
              defaultValue={currentVehicle.mileageKm ? String(currentVehicle.mileageKm) : ""}
            />
          </label>
      ...
      <section className="account-card" aria-labelledby="favoritos">
        <h2 id="favoritos">Favoritos</h2>
        {visibleFavorites.length === 0 ? (
          <p className="detail-meta">
            Todavía no guardaste ninguna unidad. Marcá las que te interesen desde
            su ficha y las vas a encontrar acá. <a href="/stock">Ver stock</a>.
          </p>
        ) : (
      ...
      <section className="account-card" aria-labelledby="busquedas">
        <h2 id="busquedas">Búsquedas guardadas</h2>
        {visibleSearches.length === 0 ? (
          <p className="detail-meta">
            Guardá una búsqueda desde el stock para volver a ella sin repetir los filtros.
          </p>
        ) : (
          <ul className="account-list">
            {visibleSearches.map((search) => (
              <li key={search.id}>
                <div>
                  <a href={`/stock?${new URLSearchParams(
                    Object.entries(search.query).map(([key, value]) => [key, String(value)]),
                  ).toString()}`}>
      ...
      <section className="account-card" aria-labelledby="seguridad">
