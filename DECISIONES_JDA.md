...
| 12 | Cuenta del puente de mensajería | JDA | ¿Quién contrata la cuenta de Zernio, con qué plan, y qué cuentas se conectan (WhatsApp, Instagram, Facebook)? | Puente y gestor de conexión implementados en `/panel/conversaciones`, con OAuth, sincronización verificada, alta/prueba de webhook e idempotencia. Vercel todavía no tiene `ZERNIO_API_KEY` ni `ZERNIO_WEBHOOK_SECRET`, por lo que sigue inactivo. | Cuenta creada con al menos una cuenta conectada y las dos claves cargadas en Preview y Production; webhook probado desde el panel. | No entra ni sale ningún mensaje: el CRM unificado queda sin canales. |
